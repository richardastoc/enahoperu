#' scraping daos de datos poblacionales
#'
#'
#' @return df poblacional
#' @export
#'
#' @examples
#' poblacion_data()
poblacion_data=function(){
  ...
  for (i in 1:length(temp_provincias)) {
    ...
    name_raw[[i]]=tibble(ubigeo=temp_name1,
                         provincia=chartr("ÁÉÍÓÚ", "AEIOU", toupper(temp_name2))
                           )
  }
  ...
  data_raw$provincia= chartr("ÁÉÍÓÚ", "AEIOU", toupper(data_raw$provincia))
